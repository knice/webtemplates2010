		...
		var options = $.extend(defaults, options);
			...
			//	$(obj).append(html);
				...
				if(o.displayThumbnails)
				{
					...
					$(obj).after('<div id="thumbs'+randID+'" style="position:relative;text-align:center;padding-top:5px;margin:0 auto;" class="thumbers"></div>');
					//define total thumb width 
					$('#thumbs'+randID).width(400);
					for(i=0;i<=numImages-1;i++)
					{
						thumb = $('img:eq('+(i+1)+')', obj).attr('src');
						$('#thumbs'+randID).append('<div class="thumb" id="thumb'+randID+'_'+(i+1)+'" style="cursor:pointer;">'+(i+1)+'</div>');
						if(i==0) $('#thumb'+randID+'_1').css({'background-color':'#c2c0b6'});
					}
					// Next two lines are a special case to handle the first list element which was originally the last
					thumb = $('img:first', obj).attr('src');
					//$('#thumb'+randID+'_'+numImages).css({'background-image':'url('+thumb+')'});
					$('#thumbs'+randID+' div.thumb:not(:first)').css({'background':'#ece9d8'}); // makes all thumbs 65% opaque except the first one
					$('#thumbs'+randID+' div.thumb').hover(function(){$(this).css({'background':'#c2c0b6'},150)},function(){if(curr!=this.id.split('_')[1]) $(this).css({'background':'#ece9d8'},250)}); // add hover to thumbs
				...
				function thumbclick(event)
				{
					...
					{
						$('#thumb'+randID+'_'+curr).css({'background-color':'#ece9d8'});
						$('#progress'+randID).stop().fadeOut();
						clearTimeout(clearInt);
				...
				{
					$('#thumbs'+randID+' div').css({'background-color':'#ece9d8'}).animate({opacity: 0.65},500);
					setTimeout(function(){elem.css({'background-color':'#c2c0b6'}).animate({'opacity': .99},500);},o.transitionSpeed);
				}
